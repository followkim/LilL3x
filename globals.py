...
def SleepOn(secs, varf=STATE.GetState, step=0.5, checkState=True):
    if checkState: thisState = STATE.GetState()
    var = varf()  # starting condition

    sleep_for = min(step, max(secs,step))
    target_time = datetime.now() + timedelta(seconds=secs)

    while var==varf() and (secs<0 or datetime.now() < target_time) and (not checkState or STATE.CheckState(thisState)):
        sleep(sleep_for)
